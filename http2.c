...
static int listen_socket(char *port) {
  ...
  log_exit("faild to listen socket");
  return -1;
}
...
static long content_length(struct HTTPRequest *req) {
  ...
  if (!val) return 0;
  len = atol(val);
  if (len < 0) log_exit("negative content_length value");
  return len;
}
...
static void read_request_line(struct HTTPRequest *req, FILE *in) {
  ...
  if (!fgets(buf, sizeof buf, in)) log_exit("no request line");
  p = strchr(buf, ' ');
